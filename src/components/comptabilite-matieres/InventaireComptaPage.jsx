...
// Inventaire — Comptabilité Matières (Fonctionnement/Non-Pharmaceutique) —
// comptage physique confronté au stock calculé. Limité à la PHARMACIE pour
// l'instant : la consommation côté service n'est pas encore tracée dans
// cette application (voir StockComptaPage — la vue service n'affiche qu'un
// cumul de ce qui a été remis, pas un stock qui se déplète), donc un
// inventaire service n'aurait rien de significatif à comparer. Cette
// rubrique pourra être étendue aux services le jour où leur consommation
// sera tracée.
export function InventaireComptaPage({store,activeSupplier,currentUser}){
  const COMPTA_CIRCUITS = getComptaCircuits(store);
  const available = availableCircuits(currentUser,"inventaire");
  const [circuit,setCircuit] = useState(available[0]||"fonctionnement");
  const cfg = COMPTA_CIRCUITS[circuit];
  const scope = circuit==="fonctionnement" ? "fonct-pharmacy" : "np-pharmacy";
  const getPharmacyStock2 = circuit==="fonctionnement" ? getFonctPharmacyStock2 : getNpPharmacyStock2;
  const createStock2Inventory = circuit==="fonctionnement" ? store.createStock2InventoryFonct : store.createStock2InventoryNp;
  const inventoriesData = circuit==="fonctionnement" ? store.stock2InventoriesFonct : store.stock2InventoriesNp;
  const permSection = circuit==="fonctionnement" ? "inventaire-fonct" : "inventaire-np";
  ...
  const scopedProducts = store.products
    .filter(p=>productVisibleInCircuit(p,circuit,store.suppliers) && (activeSupplier ? p.supplierId===activeSupplier.id : hasSupplierAccess(currentUser,p.supplierId)))
  ...
  const getComputed = (productId) => getPharmacyStock2(store,productId);
  ...
  const submit = async () => {
    ...
    setSaving(true);
    try {
      await createStock2Inventory(scope, lines);
  ...
  const scopeLines = (inventoriesData||[]).filter(d=>d.scope===scope);
  const sessionsMap = {};
  ...
  return (
    <div style={{padding:0}}>
      <PageHeader pageId={permSection} title={"🗒️ Inventaire — "+cfg.label} subtitle="Comptage physique du stock pharmacie"/>
      <div style={{padding:16}}>
        <CircuitSelector circuit={circuit} setCircuit={c=>{setCircuit(c);setMode("complet");setSelectedIds([]);setCounts({});setSearch("");}} available={available} circuits={COMPTA_CIRCUITS}/>
